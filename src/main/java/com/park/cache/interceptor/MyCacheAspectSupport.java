 ...
 * @date 2020/6/7 20:59
 */
public abstract class MyCacheAspectSupport extends AbstractCacheInvoker
    ...
    private final MyCacheOperationExpressionEvaluator evaluator = new MyCacheOperationExpressionEvaluator();
    ...
    public void configure(
        ...
        this.cacheResolver = new SingletonSupplier<>(cacheResolver,
                () -> MySimpleCacheResolver.of(SupplierUtils.resolve(cacheManager)));
    }
    ...
    public void setCacheManager(CacheManager cacheManager) {
        this.cacheResolver = SingletonSupplier.of(new MySimpleCacheResolver(cacheManager));
    }
    ...
    protected CacheOperationMetadata getCacheOperationMetadata(
        ...
        if (metadata == null) {
            ...
            if (StringUtils.hasText(operation.getCacheResolver())) {
                ...
                operationCacheResolver = new MySimpleCacheResolver(cacheManager);
            } else {
    ...
    private Object execute(final CacheOperationInvoker invoker, Method method, CacheOperationContexts contexts) {
        ...
        if (contexts.isSynchronized()) {
            CacheOperationContext context = contexts.get(AddCacheOperation.class).iterator().next();
            if (isConditionPassing(context, MyCacheOperationExpressionEvaluator.NO_RESULT)) {
                Object key = generateKey(context, MyCacheOperationExpressionEvaluator.NO_RESULT);
        ...
        processCacheEvicts(contexts.get(CacheEvictOperation.class), true,
                MyCacheOperationExpressionEvaluator.NO_RESULT);

        // Check if we have a cached item matching the conditions

        System.out.println("--------- start cache  --------");
        Cache.ValueWrapper cacheHit = findCachedItem(contexts.get(AddCacheOperation.class));
        System.out.println("--------- end cache  --------");

        // Collect puts from any @Cacheable miss, if no cached item is found
        List<CachePutRequest> cachePutRequests = new LinkedList<>();
        if (cacheHit == null) {
            collectPutRequests(contexts.get(AddCacheOperation.class),
                    MyCacheOperationExpressionEvaluator.NO_RESULT, cachePutRequests);
        }
    ...
    private boolean hasCachePut(CacheOperationContexts contexts) {
        ...
        for (CacheOperationContext context : cachePutContexts) {
            try {
                if (!context.isConditionPassing(MyCacheOperationExpressionEvaluator.RESULT_UNAVAILABLE)) {
                    excluded.add(context);
                }
            } catch (MyVariableNotAvailableException ex) {
    ...
    /**
     * Find a cached item only for {@link AddCacheOperation} that passes the condition.
     *
    ...
    private Cache.ValueWrapper findCachedItem(Collection<CacheOperationContext> contexts) {
        Object result = MyCacheOperationExpressionEvaluator.NO_RESULT;
        for (CacheOperationContext context : contexts) {
    ...
    private class CacheOperationContexts {
        ...
        private boolean determineSyncFlag(Method method) {
            List<CacheOperationContext> cacheOperationContexts = this.contexts.get(AddCacheOperation.class);
            if (cacheOperationContexts == null) {  // no @Cacheable operation at all
                return false;
            }
            boolean syncEnabled = false;
            for (CacheOperationContext cacheOperationContext : cacheOperationContexts) {
                if (((AddCacheOperation) cacheOperationContext.getOperation()).isSync()) {
                    syncEnabled = true;
                    break;
                }
            }
            if (syncEnabled) {
                ...
                AddCacheOperation operation = (AddCacheOperation) cacheOperationContext.getOperation();
    ...
    protected class CacheOperationContext implements CacheOperationInvocationContext<CacheOperation> {
        ...
        protected boolean canPutToCache(@Nullable Object value) {
            String unless = "";
            if (this.metadata.operation instanceof AddCacheOperation) {
                unless = ((AddCacheOperation) this.metadata.operation).getUnless();
